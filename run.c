...
{	Element *f, *g;
	...
	if (!e || !e->n)
		return ZE;
                ...
                // BRISHNA edits for more accurate line printing inside if/do/unless structures (with sub elements)
                if(e->sub)    // contains sub - for more accurate line printing
                {
                    printf("\n\teval_sub(%d %s: line %d) ", e->Seqno, e->esc?"+esc":"", e->n?e->sub->this->frst->n->ln:0);
                }
                else
                {
                    printf("\n\teval_sub(%d %s: line %d) ", e->Seqno, e->esc?"+esc":"", e->n?e->n->ln:0);
                }
	...
	{	Element *has_else = ZE;
		...
		{	printf("Select stmnt (");
			...
			if (nproc-nstop > 1)
			{	
				// BRISHNA reformatting "choic 0: other process" to match refular choices with, dummy values
				// printf("\tchoice 0: other process\n");	// original

				// changed
				printf("\tchoice 0: proc 0 (notareal__process:0) notareal__file:0 (state 0) [other process]\n");	// original
				// p_talk(e , 0);
				// printf("\n");
		...
		{	j++;
			...
			{	if (z->this->frst->n->ntyp == ELSE)
				{	has_else = (Rvous)?ZE:z->this->frst->nxt;
					nr_else = j;
                                        elsePrint = z->this->frst;  // BRISHNA edit
					continue;
				}
				printf("\tchoice %d: ", j);
#if 0
				if (z->this->frst->n)
					printf("line %d, ", z->this->frst->n->ln);
                                ...
                                // BRISHNA : edits for more verbose printing, to match the rest of the program
                                p_talk(z->this->frst, 0);
				...
				comment(stdout, z->this->frst->n, 0);
                                ...
                                if(z->this->frst->n->ntyp == GOTO)
                                {
                                    printGotoTarget(z->this->frst);
                                }
                                // BRISHNA : added function to print targets of Atomic section for more informed decision making
                                if(z->this->frst->n->ntyp == ATOMIC)
                                {
                                    printAtomicTarget(z->this->frst);
                                }
                                printf("]\n");
//				printf("\n");
		}	}
...
rswitchrepeatRun:		if (scanf("%64s", buf) <= 0)
		...
		{	if (z->this->frst
			...
			if (i >= k)
			{
                            if(bas_else)    // BRISHNA : set LastStep for accurate reporting for else
                                LastStep = bas_else;
					...
					{	if (verbose&4)
						{	printf("\tEscape taken (-J) ");
							if (g->n && g->n->fn)
                                                        {
                                                            // BRISHNA edits for more accurate line printing inside if/do/unless structures (with sub elements)
                                                            if(g->sub)    // contains sub - for more accurate line printing
                                                            {
                                                                    printf("%s:%d", g->n->fn->name, g->sub->this->frst->n->ln);
                                                            }
                                                            else
                                                            {
