		...
		{	if (X->pid > 255) break;
			...
			{	int unex;
				...
				if (X->pc->esc) printf(" + Escape");
                                // BRISHNA : added function to print targets of Atomic section for more informed decision making
                                if(X->pc->n->ntyp == ATOMIC)
                                {
                                    printAtomicTarget(X->pc);
                                }
				printf("]\n");
			} else {
			has_else = ZE;
			...
			proc_k = k;
                        // BRISHNA : edit : adding variable to store ELSE element for printing after loop
                        Element *elsePrint;
			...
			{	Element *y = silent_moves(z->this->frst);
				...
				printf("\tchoice %d: ", k++);
                                // BRISHNA : Edits for more accurate line number printing. Formerly printed line number of 1st option for all options
//				p_talk(X->pc, 0);   // was original
                                p_talk(y, 0);       // for accuracy
				if (unex)
					printf(" unexecutable,");
				printf(" [");
				...
				printf("]\n");
                                ...
                                // BRISHNA : added function to print IF/DO's sub elements for more informed decision making
                                if(y->n->ntyp == IF || y->n->ntyp == DO)
                                {
                                    printSubConditions(y);
                                }
                                
                                
                                // BRISHNA : added function to print targets of GOTO for more informed decision making
                                if(y->n->ntyp == GOTO)
                                {
                                    printGotoTarget(y);
                                }
			...
			{	if (no_choice-proc_no_ch >= (k-proc_k)-1)
				{	only_choice = nr_else;
					printf("\tchoice %d: ", nr_else);
                                        // BRISHNA : edit for more accurate line number printing for ELSE
//                                        p_talk(X->pc, 0);
					p_talk(elsePrint, 0);
...
//                                        p_talk(X->pc, 0);
		...
		{	if (k - no_choice < 2)
			{	printf("no executable choices\n");
                                // BRISHNA : addition to retrieve entire output from interactive mode
                                char brirbuf[5];
                                fflush(stdout); // BRISHNA ADDITION

        rswitchrepeatExSched:	if (scanf("%64s", brirbuf) > 0)
                                {	
                                    if (brirbuf[0] == 'r')
                                    {
                                        printf("###allsent###");
                                        fflush(stdout);
                                        goto rswitchrepeatExSched;
                                    }
                                } 
				alldone(0);
			}
...
rswitchrepeatSched:	if (scanf("%64s", buf) == 0)
			{	printf("\tno input\n");
				goto try_again;
			}

                        // else    // BRISHNA : testing
                        // {
			...
			{	if (X == oX)
				...
				{	                                       
					...
					comment(stdout, LastStep->n, 0);
                                        // BRISHNA : function to print line number(s) of target(s) of goto when reporting previous taken step
                                        if(LastStep->n->ntyp == GOTO)
                                            printGotoTarget(LastStep);
                                        // BRISHNA : added function to print targets of Atomic section for more informed decision making
                                        if(LastStep->n->ntyp == ATOMIC)
                                        {
                                            printAtomicTarget(LastStep);
                                        }
					printf("]\n");
                                        // BRISHNA : insert if do options here for reported steps

				}
				if (verbose&1) dumpglobals();
				if (verbose&1) printLocalAliases(X);                        
...
rswitchrepeatWrapupSched:
            if (scanf("%64s", brirbufw) > 0)
            {	
                if (brirbufw[0] == 'r')
                {
                    printf("###allsent###");
                    fflush(stdout);
                    goto rswitchrepeatWrapupSched;
                }
            } 
...
talk(RunList *r)
{
	...
	{	p_talk(r->pc, 1);
		printf("\n");
		if (verbose&1) dumpglobals();
                // if (verbose&1) printLocalAliases(r); 
